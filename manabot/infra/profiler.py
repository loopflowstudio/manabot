...
class TimingNode:
    def __init__(
        self, label: str, parent: Optional["TimingNode"] = None, max_samples: int = 100
    ):
    ...
    def track(self, label: str):
        ...
        try:
            ...
            if self.stack and self.stack[-1] == node:
                ...
                raise RuntimeError(
                    f"Profiler stack corruption detected while stopping '{full_label}'"
                )
    ...
    def get_stats(self) -> Dict[str, Dict[str, float]]:
        ...
        for full_path, node in self.node_cache.items():
            ...
            if samples:
                ...
                p5 = (
                    float(np.percentile(samples_array, 5))
                    if len(samples) > 1
                    else min_val
                )
                p95 = (
                    float(np.percentile(samples_array, 95))
                    if len(samples) > 1
                    else max_val
                )
            else:
