...
5. **CI workflow.** Add a GitHub Actions workflow that runs this same gate on
   PR/push with a full matrix/quality gate:
   - OS matrix: `ubuntu-latest`, `macos-latest`
   - `cargo fmt --all -- --check`
   - `cargo clippy --all-targets --all-features -- -D warnings`
   - `cargo test` (managym)
   - Python setup + `maturin develop --features python`
   - `pytest tests/env/ tests/agent/`
   - enum smoke: `python -c "import managym; assert int(managym.ZoneEnum.STACK) == 4"`
   - dependency caching to keep runtime manageable:
     - `Swatinem/rust-cache@v2` for cargo registry/git/target
     - setup-python pip/uv cache keyed by Python + lock/config files
     - shared target reuse so `maturin develop` does incremental Rust builds
...
All pass, zero test modifications, and the same checks run in CI.
