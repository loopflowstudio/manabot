...
Usage:
    ...
    python scripts/bench_breakdown.py --mode zero-copy
"""
...
def run_breakdown_legacy(
    num_envs: int, total_steps: int, with_inference: bool, seed: int
):
    """Profile the legacy path: Rust step → Python encode → stack."""
    ...
    # Bootstrap last_raw_obs from a reset for reward computation
    reset_results = env._rust_env.step([0] * num_envs)
    last_raw_obs = [o for o, _r, _d, _c, _i in reset_results]
        ...
        for _ in range(total_steps):
            with profiler.track("step"):
                ...
                with profiler.track("unpack"):
                    raw_obs = []
                    for env_index, (o, raw_reward, done, cut, info) in enumerate(
                        results
                    ):
                        info_dict = dict(info)
                        env.reward.compute(raw_reward, last_raw_obs[env_index], o)
...
def run_breakdown_zerocopy(
    num_envs: int, total_steps: int, with_inference: bool, seed: int
):
    """Profile the zero-copy path: step_into_buffers → sync tensors."""
    profiler = Profiler(enabled=True)
    match = Match()
    obs_space = ObservationSpace()
    reward = Reward(RewardHypers())
...
def run_breakdown(
    ...
    if mode == "zero-copy":
        profiler = run_breakdown_zerocopy(num_envs, total_steps, with_inference, seed)
    else:
        profiler = run_breakdown_legacy(num_envs, total_steps, with_inference, seed)
    ...
    print(f"\n{'=' * 72}")
    print(
        f"Breakdown [{mode}]: num_envs={num_envs}, steps={total_steps}, "
    ...
    for path in sorted(stats.keys()):
        ...
        label = path.split("/")[-1]
        print(
            f"{indent}{label:<{40 - 2 * depth}s} {total_s:>7.3f}s {pct:>5.1f}% "
...
def main():
    ...
    parser.add_argument(
        "--with-inference", action="store_true", help="Include torch forward pass"
    )
    parser.add_argument(
        "--mode",
        default="legacy",
        choices=["legacy", "zero-copy"],
        help="Which step path to profile",
    )
    parser.add_argument("--seed", type=int, default=42)
    args = parser.parse_args()

    run_breakdown(args.num_envs, args.steps, args.with_inference, args.seed, args.mode)
